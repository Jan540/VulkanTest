...
private:
    ...
    void initWindow()
    {
        ...
        window = glfwCreateWindow(WIDTH, HEIGHT, "IS THIS A TREIANGLE?", nullptr, nullptr);
    }
    ...
    void createInstance()
    {
        ...
        appInfo.pApplicationName = "Crazy Triangle";
    ...
    void pickPhysicalDevice()
    {
        ...
        if (deviceCount == 0)
        {
            throw std::runtime_error("failed to find GPU with Vulkan suport :(");
        }

        std::vector<VkPhysicalDevice> devices(deviceCount);
        vkEnumeratePhysicalDevices(instance, &deviceCount, devices.data());

        std::multimap<int, VkPhysicalDevice> candiates;

        for (const auto &device : devices)
        {
            int score = rateDeviceSuitability(device);
            candiates.insert(std::make_pair(score, device));
        }

        if (candiates.rbegin()->first > 0)
        {
            physicalDevice = candiates.rbegin()->second;
        }
        else
        {
    ...
    void createLogicalDevice()
    {
        ...
        std::set<uint32_t> uniqueQeueuFamilies = {indices.graphicsFamily.value(),
